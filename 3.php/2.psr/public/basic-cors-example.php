...
try {
    ...
    // Note that the static routes should be on top of the dynamic:
    // https://github.com/nikic/FastRoute/issues/80
    $router->group('/api', function (\League\Route\RouteGroup $route) {
        $route->get('/users', function (ServerRequestInterface $request) : ResponseInterface {
            return new JsonResponse('Hello world!');
        });
        $route->map('GET', '/{routes:.+}', function (ServerRequestInterface $request) : ResponseInterface {
